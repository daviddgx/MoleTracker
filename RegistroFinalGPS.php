...
if ($conn->connect_error) {
    ...
    $QryGuiaActiva = "SELECT * FROM traker_mole.trck_mle_guias where GuiaSAP ='" . $_SESSION["GuiaSAPActiva"] . "' and Estatus <> 'Entregado'";
    ...
    try {
        ...
        while ($rowPiloto = $EsUsuarioPiloto->fetch_assoc()!== null) {
            ...
            $ActualizarGuiaProceso = "insert into traker_mole.trck_mle_reg_guias values (0,'" . $_SESSION["GuiaSAPActiva"] . "','','" . $rowPiloto['Piloto'] . "','Entregado','" . $_SESSION['Usuario'] . "','" . $_SESSION["FechaTrabajo"] . " " . $_SESSION["HoraTrabajo"] . "',0,0, 'Entregado' )";

            if ($conn->query($ActualizarGuiaProceso) === TRUE) {

                $ActualizarGuia = "update traker_mole.trck_mle_guias set Estatus='Entregado', Tracker_Final = '" . $Latitud . ", " . $Longitud . "' where GuiaSAP = '" . $_SESSION["GuiaSAPActiva"] . "' ";
